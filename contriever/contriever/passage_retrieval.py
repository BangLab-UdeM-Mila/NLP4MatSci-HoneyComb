...
def main(args):
    ...
    for path in data_paths:
        data = load_data(path)
        saved_filename_suffix = os.path.basename(path)[os.path.basename(path).find("."):]
        saved_filename = os.path.basename(path).replace(saved_filename_suffix, "_with_top_k_docs"+saved_filename_suffix)
        output_path = os.path.join(args.output_dir, saved_filename)
        ...
        with open(output_path, "w") as fout:
            for ex in data:
                # Modified:
                # Control top k most similar docs returned
                ex['ctxs'] = ex['ctxs'][:args.top_k_returned_docs]
...
if __name__ == "__main__":
    ...
    #Modified - newly added arg
    parser.add_argument("--top_k_returned_docs", type=int, default=10, help="Number of top_k most similar docs to save per question")
